...
_mimxrt595_user_overrides = {
    "//command_line_option:platforms": str(Label("@pigweed//targets/mimxrt595_evk_freertos:platform")),
    "@pigweed//pw_system:extra_platform_libs": str(Label(":user_extra_platform_libs")),
}

def _mimxrt595_user_transition_impl(settings, attr):
    # buildifier: disable=unused-variable
    _ignore = settings, attr

    return merge_flags_for_transition_impl(
        base = MIMXRT595_SYSTEM_FLAGS,
        override = _mimxrt595_user_overrides,
    )

_mimxrt595_user_transition = transition(
    implementation = _mimxrt595_user_transition_impl,
    inputs = [],
