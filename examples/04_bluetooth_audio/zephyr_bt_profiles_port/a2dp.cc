...
//
///* Connections */
static struct bt_a2dp connection[1];
//
...
#endif
int a2dp_get_capabilities_ind(struct bt_avdtp *session, struct bt_avdtp_sep *sep,
...
int a2dp_set_config_ind(struct bt_avdtp *session, struct bt_avdtp_sep *sep,
		...
		struct net_buf *buf)
{
	...
	media_hdr = (struct bt_avdtp_media_hdr *)net_buf_pull_mem(buf, sizeof(*media_hdr));
...
int a2dp_open_ind(struct bt_avdtp *session, struct bt_avdtp_sep *sep, uint8_t *errcode)
{
    (void)session;
	struct bt_a2dp_ep *ep;
	struct bt_a2dp_stream *stream;
...
int a2dp_start_ind(struct bt_avdtp *session, struct bt_avdtp_sep *sep, uint8_t *errcode)
{
    (void)session;
	struct bt_a2dp_ep *ep;
	struct bt_a2dp_stream *stream;
...
//}
//
int a2dp_suspend_ind(struct bt_avdtp *session, struct bt_avdtp_sep *sep, uint8_t *errcode)
{
    (void)session;
	struct bt_a2dp_ep *ep;
	struct bt_a2dp_stream *stream;
...
int a2dp_accept(struct bt_conn *conn, struct bt_avdtp **session)
{
	...
	*session = &(a2dp->session);
	...
	PW_LOG_DEBUG("session: %p", &(a2dp->session));
...
int bt_a2dp_init_zephyr(void)
{
	...
	if (err < 0) {
		PW_LOG_CRITICAL("A2DP registration failed");
		return err;
	}
	...
	PW_LOG_DEBUG("A2DP Initialized successfully.");
	return 0;
}
