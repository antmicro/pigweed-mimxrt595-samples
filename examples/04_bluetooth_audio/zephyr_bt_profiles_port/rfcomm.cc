...
#define DLC_RTX(_w) \
  CONTAINER_OF(k_work_delayable_from_work(_w), struct bt_rfcomm_dlc, rtx_work)
#define SESSION_RTX(_w) \
  CONTAINER_OF(         \
      k_work_delayable_from_work(_w), struct bt_rfcomm_session, rtx_work)
...
static bool rfcomm_check_fcs(uint16_t len,
                             const uint8_t* data,
                             uint8_t recvd_fcs) {
  uint8_t fcs = 0xff;
...
// }
//
static int rfcomm_send_cb(struct bt_rfcomm_session* session,
                          struct net_buf* buf,
                          bt_conn_tx_cb_t cb,
                          void* user_data) {
  int err = 0;
  ...
  for (unsigned int i = 0; i < buf->len; i++) {
    ...
    LOG_DBG("Sending buf->data[%d] = 0x%02x", i, buf->data[i]);
  }
  session->br_chan.chan.chan->Send(std::move(ptr));
...
static struct bt_rfcomm_dlc* rfcomm_dlc_accept(
...
// }
//
// static void bt_rfcomm_tx_destroy(struct bt_rfcomm_dlc *dlc, struct net_buf
// *buf)
//{
...
static void rfcomm_dlc_tx_thread(void* p1, void* p2, void* p3) {
  ...
  struct net_buf* buf = nullptr;

  LOG_DBG("Started for dlc %p, state: %d", dlc, dlc->state);
  ...
  while (dlc->state == BT_RFCOMM_STATE_CONNECTED ||
    ...
    if (buf && rfcomm_send_cb(dlc->session, buf, rfcomm_sent, dlc) < 0) {
...
static int rfcomm_send_msc(struct bt_rfcomm_dlc* dlc,
                           uint8_t cr,
                           uint8_t v24_signal) {
...
static int rfcomm_send_rls(struct bt_rfcomm_dlc* dlc,
                           uint8_t cr,
                           uint8_t line_status) {
...
static int rfcomm_send_rpn(struct bt_rfcomm_session* session,
                           uint8_t cr,
...
static int rfcomm_send_test(struct bt_rfcomm_session* session,
                            uint8_t cr,
                            uint8_t* pattern,
                            uint8_t len) {
  struct net_buf* buf;
...
static int rfcomm_send_nsc(struct bt_rfcomm_session* session,
                           uint8_t cmd_type) {
  struct net_buf* buf;
  uint8_t fcs;

  buf = rfcomm_make_uih_msg(
      session, BT_RFCOMM_MSG_RESP_CR, BT_RFCOMM_NSC, sizeof(cmd_type));
...
static void rfcomm_dlc_connected(struct bt_rfcomm_dlc* dlc) {
  ...
  // pw::thread::DetachedThread(pw::thread::freertos::Options(), [dlc]() {
  //      rfcomm_dlc_tx_thread(dlc, NULL, NULL);
  //  });
...
static enum security_result rfcomm_dlc_security(struct bt_rfcomm_dlc* dlc) {
  ...
  ///*Security request failed*/ return RFCOMM_SECURITY_REJECT;
}
...
static void rfcomm_handle_sabm(struct bt_rfcomm_session* session,
                               uint8_t dlci) {
  if (!dlci) {
...
static void rfcomm_handle_disc(struct bt_rfcomm_session* session,
                               uint8_t dlci) {
  struct bt_rfcomm_dlc* dlc;
...
// }
//
int rfcomm_recv(struct bt_l2cap_chan* chan, struct net_buf* buf) {
  struct bt_rfcomm_session* session = RFCOMM_SESSION(chan);
  ...
  if (BT_RFCOMM_LEN_EXTENDED(hdr->length)) {
    msg_len =
        BT_RFCOMM_GET_LEN_EXTENDED(hdr_ext->hdr.length, hdr_ext->second_length);
  ...
  if (buf->len < (hdr_len + msg_len + sizeof(fcs))) {
    LOG_ERR("Too small RFCOMM information (%d < %d)",
            buf->len,
...
static struct bt_rfcomm_session* rfcomm_session_new(bt_rfcomm_role_t role) {
  ...
  for (i = 0; i < ARRAY_SIZE(bt_rfcomm_pool); i++) {
    ...
    session->mtu = CONFIG_BT_RFCOMM_L2CAP_MTU;
...
// }
//
int rfcomm_accept(struct bt_conn* conn,
