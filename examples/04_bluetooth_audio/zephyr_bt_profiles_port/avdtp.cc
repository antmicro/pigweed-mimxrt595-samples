...
static pw::Vector<struct bt_avdtp_sep*, 12> seps;

#define AVDTP_CHAN(_ch) CONTAINER_OF(_ch, struct bt_avdtp, br_chan.chan)

#define AVDTP_KWORK(_work)                                         \
  CONTAINER_OF(CONTAINER_OF(_work, struct k_work_delayable, work), \
               struct bt_avdtp,                                    \
               timeout_work)
//
// #define DISCOVER_REQ(_req) CONTAINER_OF(_req, struct
// bt_avdtp_discover_params, req) #define GET_CAP_REQ(_req) CONTAINER_OF(_req,
// struct bt_avdtp_get_capabilities_params, req) #define SET_CONF_REQ(_req)
// CONTAINER_OF(_req, struct bt_avdtp_set_configuration_params, req) #define
// OPEN_REQ(_req) CONTAINER_OF(_req, struct bt_avdtp_open_params, req) #define
// START_REQ(_req) CONTAINER_OF(_req, struct bt_avdtp_start_params, req)
//
...
// void bt_avdtp_media_l2cap_connected(bt::l2cap::Channel::WeakPtr chan) {
...
static struct net_buf* avdtp_create_reply_pdu(uint8_t msg_type,
  ...
  hdr = (struct bt_avdtp_single_sig_hdr*)net_buf_add(buf, sizeof(*hdr));

  hdr->hdr =
      (msg_type | pkt_type << AVDTP_PKT_POSITION | tid << AVDTP_TID_POSITION);
  hdr->signal_id = sig_id & AVDTP_SIGID_MASK;

  PW_LOG_DEBUG("hdr = 0x%02X, Signal_ID = 0x%02X", hdr->hdr, hdr->signal_id);
  return buf;
}
...
static void avdtp_discover_handler(struct bt_avdtp* session,
  ...
  if (msg_type == BT_AVDTP_CMD) {
    ...
    if (err) {
      ...
      PW_LOG_DEBUG("discover err code:%d", error_code);
      net_buf_add_u8(rsp_buf, error_code);
    } else {
      for (auto s : seps) {
        uint8_t p1 = ((s->sep_info.id << 2) & 0xfc) | s->sep_info.inuse << 1;
        uint8_t p2 = ((s->sep_info.media_type << 4) & 0xf0) | s->sep_info.tsep
                                                                  << 3;
        net_buf_add_u8(rsp_buf, p1);
        net_buf_add_u8(rsp_buf, p2);
        // p1 = ((s.sep_info.id << 2) & 0xfc) | s.sep_info.inuse << 1;
        // p2 = ((s.sep_info.media_type << 4) & 0xf0) | 0 << 3;
        // net_buf_add_u8(rsp_buf, p1);
        // net_buf_add_u8(rsp_buf, p2);
      }
    }
    ...
    if (err < 0) {
      ...
      PW_LOG_CRITICAL("Error:L2CAP send fail - result = %d", err);
      return;
    }
  } else {
    // struct bt_avdtp_req *req = session->req;
...
//		if ((sep == NULL) || (session->ops->get_capabilities_ind ==
// NULL)) { 			err = -ENOTSUP; 		} else {
...
//			}
//		} else if (msg_type == BT_AVDTP_REJECT) {
//			GET_CAP_REQ(session->req)->status =
// net_buf_pull_u8(buf); 		} else if (msg_type ==
// BT_AVDTP_GEN_REJECT) { GET_CAP_REQ(session->req)->status =
...
static void avdtp_get_all_capabilities_handler(struct bt_avdtp* session,
  ...
  if (msg_type == BT_AVDTP_CMD) {
    ...
    if ((sep == NULL) || (session->ops->get_capabilities_ind == NULL)) {
      ...
      err = session->ops->get_capabilities_ind(
          session, sep, rsp_buf, &error_code);
      if (err) {
        net_buf_unref(rsp_buf);
      }
    }
    ...
    if (err) {
      ...
      PW_LOG_DEBUG("get cap err code:%d", error_code);
      net_buf_add_u8(rsp_buf, error_code);
    }
...
int bt_avdtp_parse_capability_codec(struct net_buf* buf,
  ...
  if (!buf) {
    PW_LOG_DEBUG("Error: buf not valid");
    return -EINVAL;
  }
  ...
  while (buf->len()) {
    ...
    switch (data) {
      ...
      case BT_AVDTP_SERVICE_MEDIA_CODEC:
        ...
        if (length > 3) {
          data = net_buf_pull_u8(buf);
          if (net_buf_tailroom(buf) < (size_t)(length - 1)) {
            return -EINVAL;
          }
          if (data == BT_AVDTP_AUDIO) {
            ...
            *codec_info_element =
                (uint8_t*)net_buf_pull_mem(buf, (*codec_info_element_len));
            return 0;
          }
        }
        break;
  ...
  if (msg_type == BT_AVDTP_CMD) {
    ...
    if ((sep == NULL) || (session->ops->set_configuration_ind == NULL)) {
      ...
      if (sep->state == AVDTP_STREAMING) {
        ...
        PW_LOG_DEBUG("a2dp_set_config_ind err = %d", err);
      }
    }
    ...
    if (err) {
      ...
      PW_LOG_DEBUG("set configuration err code:%d", error_code);
...
static void avdtp_open_handler(struct bt_avdtp* session,
  ...
  if (msg_type == BT_AVDTP_CMD) {
    ...
    if (err) {
      ...
      PW_LOG_DEBUG("open_ind err code:%d", error_code);
      net_buf_add_u8(rsp_buf, error_code);
    } else {
...
static void avdtp_start_handler(struct bt_avdtp* session,
  ...
  if (msg_type == BT_AVDTP_CMD) {
    ...
    if (err) {
      ...
      PW_LOG_DEBUG("start err code:%d", error_code);
      net_buf_add_u8(rsp_buf, error_code);
    } else {
...
//				error_code = BT_AVDTP_BAD_STATE;
//			} else {
//				err = session->ops->close_ind(session, sep,
//&error_code);
//			}
//		}
//
...
static void avdtp_suspend_handler(struct bt_avdtp* session,
  ...
  if (msg_type == BT_AVDTP_CMD) {
    ...
    if (err) {
      ...
      PW_LOG_DEBUG("suspend err code:%d", error_code);
      net_buf_add_u8(rsp_buf, error_code);
    } else {
      sep->state = AVDTP_OPEN;
    }
...
//			err = -ENOTSUP;
//		} else {
//			err = session->ops->abort_ind(session, sep,
//&error_code);
//		}
//
//		rsp_buf = avdtp_create_reply_pdu(err ?
...
// }
//
// void bt_avdtp_l2cap_encrypt_changed(struct bt_l2cap_chan *chan, uint8_t
// status)
//{
//	LOG_DBG("");
// }
...
static const struct {
  uint8_t sig_id;
  void (*func)(struct bt_avdtp* session,
               struct net_buf* buf,
               uint8_t msg_type,
               uint8_t tid);
} handler[] = {
    ...
    {BT_AVDTP_GET_ALL_CAPABILITIES, avdtp_get_all_capabilities_handler},
...
int bt_avdtp_l2cap_recv(struct bt_l2cap_chan* chan, struct net_buf* buf) {
  ...
  PW_LOG_DEBUG("pack_type[0x%02x] msg_type[0x%02x] sig_id[0x%02x] tid[0x%02x]",
               pack_type,
               msgtype,
               sigid,
               tid);
  ...
  if (msgtype != BT_AVDTP_CMD) {
    // if (session->req == NULL) {
    //	PW_LOG_DEBUG("Unexpected peer response");
    //	return 0;
    // }

    // if (session->req->sig != sigid ||
    //	session->req->tid != tid) {
    //	PW_LOG_DEBUG("Peer mismatch resp, expected sig[0x%02x]"
  ...
  // if (!session) {
  //	PW_LOG_DEBUG("Error: Session not valid");
  //	return 0;
  // }

  for (i = 0U; i < PW_ARRAY_SIZE(handler); i++) {
...
// }
//
// int bt_avdtp_parse_sep(struct net_buf *buf, struct bt_avdtp_sep_info
// *sep_info)
//{
...
//	/* Codec Info Element */
//	net_buf_add_mem(buf, param->codec_specific_ie,
...
//	}
//
//	return avdtp_process_configure_command(session,
// BT_AVDTP_SET_CONFIGURATION, param);
// }
//
