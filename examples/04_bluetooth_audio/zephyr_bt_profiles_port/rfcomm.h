...
///** RFCOMM Maximum Header Size. The length could be 2 bytes, it depends on
///information length. */
...
// */
// #define BT_RFCOMM_BUF_SIZE(mtu) \ 	BT_L2CAP_BUF_SIZE(BT_RFCOMM_HDR_MAX_SIZE +
//BT_RFCOMM_FCS_SIZE + (mtu))
//
...
struct bt_rfcomm_server {
   ...
   *
   *  0x01 - 0x1e Dynamically allocated. May be pre-set by the application
   * before server registration (not recommended however), or auto-allocated by
   * the stack if the 0 is passed.
   */
  uint8_t channel;
...
// *  using the accept() callback which in case of success shall allocate the
// dlc
