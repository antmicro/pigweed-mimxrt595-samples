				    ...
				    size_t len)
{
    ...
    {
        buf->data.push_back(((const uint8_t*)mem)[i]);
    }
    return buf->data.end();
}
...
//}
//
static inline struct net_buf * bt_l2cap_create_pdu(void *, int) {
    struct net_buf * buf = new struct net_buf();
...
{
    uint8_t *data = (uint8_t*)malloc(len * sizeof(int8_t));
    std::memcpy(data, buf->data.data(), len);
...
{
    return *(uint8_t*)net_buf_pull_mem(buf, 1);
}
//
///**
// * @brief Remove and convert 16 bits from the beginning of the buffer.
...
{
	return buf->data.capacity() - buf->data.size();
}
//
///**
// * @brief Check buffer headroom.
